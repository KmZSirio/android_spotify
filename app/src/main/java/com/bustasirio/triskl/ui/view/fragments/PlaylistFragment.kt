...
import com.bustasirio.triskl.databinding.FragmentPlaylistBinding
import com.bustasirio.triskl.ui.view.adapters.PlaylistAdapter
import com.bustasirio.triskl.ui.viewmodel.PlaylistViewModel
...
class PlaylistFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.fabPlaylist.setOnClickListener { showToast(requireContext(), getString(R.string.not_working)) }
        ...
        // * TracksResponse
        playlistVM.tracksResponse.observe(viewLifecycleOwner, { response ->
            when (response) {
                is Resource.Success -> {
                    hideProgressBar(view)

                    playlistAdapter.differ.submitList(response.data!!.items.toList())
                    val totalPages = response.data.total / QUERY_SIZE + 1
                    isLastPage = playlistVM.page == totalPages

                    if (response.data.items.isEmpty()) binding.ivEmptyPlaylist.visibility = View.VISIBLE
                }
